...
function lgsndmsg(uid, cookieValue, targetUser, message) {
	...
	return new Promise((resolve) => {
		...
		if (!isSending) {
			...
			console.log(`   Request queued. ${pendingRequests.length - 1} request(s) waiting.`);
		}
	});
}
	...
	return new Promise((resolve) => {
		...
		makeRequest({
			...
			if (homeResponse.data.includes('登录') || homeResponse.data.includes('Sign in')) {
				...
				resolve({ success: false, error: 'Cookies无效或已过期' });
				return { stop: true };
			}
			...
			if (csrfTokenMatch && csrfTokenMatch[1]) {
				...
				if (csrfMatch2 && csrfMatch2[1]) {
					...
					resolve({ success: false, error: '无法获取CSRF令牌' });
					return { stop: true };
				}
			}
			...
			if (response.statusCode === 200) {
				...
				resolve({ success: false, error: '请求过于频繁，请稍后重试', statusCode: response.statusCode });
			}
			else if (response.statusCode === 401 || response.statusCode === 403) {
				console.error('   Error: Authentication failed. Check your cookies and CSRF token.');
				resolve({ success: false, error: '认证失败，请检查Cookie和CSRF令牌', statusCode: response.statusCode });
			}
			else {
				...
				try {
					...
					resolve({ success: false, error: errorData.error || '发送失败', statusCode: response.statusCode, details: errorData });
				}
				catch (e) {
					console.error(`   Response: ${response.data.substring(0, 200)}`);
					resolve({ success: false, error: `发送失败，状态码：${response.statusCode}`, statusCode: response.statusCode });
				}
			}
		}).catch(() => {
			resolve({ success: false, error: '内部错误' });
		});
	});
}
